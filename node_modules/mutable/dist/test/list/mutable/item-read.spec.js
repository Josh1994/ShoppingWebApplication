...
function testReadFunctionality(builders, isReadonly) {
    describe('at', function () {
        ...
        it('should return a typed item form multiple types if there is _type field', function () {
            ...
            chai_1.expect(arr.at(0)).to.be.instanceOf(builders.UserType);
            chai_1.expect(arr.at(1)).to.be.instanceOf(builders.AddressType);
        });
        it('Should handle multi level List', function () {
            ...
            chai_1.expect(userWithAddress).to.be.instanceof(builders.UserWithAddressType);
            chai_1.expect(userWithAddress.$isReadOnly()).to.equal(isReadonly);
        });
        if (isReadonly) {
            it('should return read-only inner complex data', function () {
                var arrComplexType = builders.aUserWithAddressTypeList([{}]);
                chai_1.expect(arrComplexType.at(0).$isReadOnly()).to.be.true;
            });
        }
        else {
            it('should modify inner complex data', function () {
                ...
                chai_1.expect(arrComplexType.at(0).user.name).to.equal('modified user name');
            });
            it('should keep read only item as read only', function () {
                ...
                chai_1.expect(arrComplexType.at(0).$isReadOnly()).to.eql(true);
            });
        }
    });
    describe('indexOf', function () {
        ...
        it('should return the first index of an element within a List equal to the specified value', function () {
            chai_1.expect(stringList.indexOf('a')).to.eql(0);
        });
        it('should return -1 if none is found', function () {
            chai_1.expect(stringList.indexOf('a', 1)).to.eql(-1);
        });
    });
    describe('lastIndexOf', function () {
        ...
        it('should return the last index at which a given element can be found in the List', function () {
            chai_1.expect(stringList.lastIndexOf('a')).to.eql(2);
        });
        it('should take a 2nd argument, the index at which to start searching backwards', function () {
            chai_1.expect(stringList.lastIndexOf('b', 2)).to.eql(1);
        });
        it('should return -1 if none is found', function () {
            chai_1.expect(stringList.lastIndexOf('c')).to.eql(-1);
        });
    });
}
...
//# sourceMappingURL=item-read.spec.js.map